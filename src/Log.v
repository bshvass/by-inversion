...
Lemma log_Rpower n x (ngt1 : 1 < n) : log n (Rpower n x) = x.
Proof.
  unfold log. unfold Rpower. rewrite ln_exp. field.
  intros contra. replace 0 with (ln 1) in contra by (apply ln_1).
  apply ln_inv in contra. lra. lra. lra. Qed.
...
Proof.
  intros. unfold log. apply Rlt_div. apply ln_gt_0. assumption.
  apply ln_increasing. lra. lra. Qed.
...
Proof.
  ...
  assert (0 < ln n). apply ln_gt_0; assumption. nra. Qed.

Lemma Rpower_lt_inv n a b (nge1 : 1 <= n) : Rpower n a < Rpower n b -> a < b.
Proof.
  intros. unfold Rpower in H. apply exp_lt_inv in H.
  assert (0 <= ln n). apply ln_ge_0; assumption. nra. Qed.

Lemma log_lower_bound n a b (ngt1 : 1 < n) :
  Rpower n a < b -> a < log n b.
Proof.
  intros.
  destruct (Rle_dec b 0). pose proof Rpower_pos_nonneg n a. lra.
  replace b with (Rpower n (log n b)) in H.
  apply Rpower_lt_inv in H. assumption. lra. apply Rpower_log. lra. assumption. Qed.
...
Proof.
  ...
  destruct (Rle_dec b 0). pose proof Rpower_pos_nonneg n a. lra.
  replace b with (Rpower n (log n b)) in H.
  apply Rpower_le_inv in H. assumption. lra. apply Rpower_log. lra. assumption. Qed.

Lemma log_upper_bound n a b (a_nonneg : 0 < a) (ngt1 : 1 < n) :
  a < Rpower n b -> log n a < b.
Proof.
  intros. rewrite <- (log_Rpower n b). apply log_lt. assumption.  lra. assumption. Qed.

Lemma log_le_upper_bound n a b (a_nonneg : 0 < a) (ngt1 : 1 < n) :
  a <= Rpower n b -> log n a <= b.
Proof.
  intros. rewrite <- (log_Rpower n b). apply log_inc. lra. lra. assumption. Qed.
