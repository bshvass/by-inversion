...
From BY Require Import Matrix MatrixQ Hierarchy.
...
Definition has_at_most_norm (P : matQ) N :=
  let '(a, b, c, d) := P in
  let X := (2 * N ^ 2 - a - d) in
  (0 <=? X) && ((a - d) ^ 2 + 4 * b ^ 2 <=? X ^ 2).
...
Definition Qmat2Rmat (m : matQ) : @mat R :=
...
Inductive inSQ_gen_scaled {w0 P0 e0} : Z -> matQ -> Prop :=
| IS_gen_scaled : inSQ_gen_scaled w0 P0
| Sc_gen_scaled (w : Z) (P : matQ) :
    forall (e : Z) (q : Z),
    inSQ_gen_scaled w P ->
...
Proof.
  ...
  -
    ...
    simpl in *. nra. reflexivity. right. lia. right. lia. right. lia.
    right.  lia. right. lia.
  ...
  -
    ...
    rewrite <- !pow_powerRZ in *. simpl. nra. reflexivity. right. lia.
